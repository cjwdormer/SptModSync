    ...
    public sealed class StagedStore
    {
        ...
        public void Save(PendingOperations pending)
        {
            try
            {
                ...
                _log($"[TCF-ModSync] Kept {pending.Operations.Count} staged file(s) for next launch.");
            }
            catch (Exception ex)
            {
                _log($"[TCF-ModSync] Could not record staged downloads: {ex.Message}");
            }
        }
        ...
        public PendingOperations? Load()
        {
            ...
            {
                _log($"[TCF-ModSync] Could not read staged downloads ({ex.Message}); they will be downloaded again.");
                Discard();
                return null;
            }
        }
