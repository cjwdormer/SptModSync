...
namespace TCFModSync.Client.Handoff
{
    public sealed class UpdaterLauncher
    {
        private const string PendingFileName = "TCFModSync.pending.json";
        ...
        public void LaunchUpdaterAndPrepareForExit(PendingOperations pending)
        {
            ...
            if (relaunchingGame)
            {
                ...
                {
                    _log("[TCF-ModSync] WARNING: could not read this process's launch arguments. The game " +
            ...
            var updaterExe = Path.Combine(_gameRootDirectory, "TCFModSync.Updater.exe");
            if (!File.Exists(updaterExe))
            {
                _log($"[TCF-ModSync] Cannot find updater at '{updaterExe}' - files were downloaded to staging " +
            ...
            _log($"[TCF-ModSync] Launching updater (pid {pending.WaitForProcessId} will be awaited)...");
            Process.Start(psi);
        }
