...
namespace SptModSync.Server.Http;
...
public class SptRouteListener(ISptLogger<SptRouteListener> logger) : IHttpListener
{
    ...
    public async Task Handle(MongoId sessionId, HttpContext context)
    {
        if (Handler == null)
        {
            const string message = "[SptModSync] Request received but the mod did not start successfully; " +
        ...
        {
            var message = $"[SptModSync] Error handling {context.Request.Path}: {ex}";
    ...
    private async Task HandleFileAsync(HttpContext context, string path)
    {
        ...
        {
            var message = $"[SptModSync] Refused request for '{relativePath}' - outside the served root.";
            logger.Warning(message);
            FileLog.Current?.Write($"WARN: {message}");
