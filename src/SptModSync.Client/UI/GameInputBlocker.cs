...
namespace SptModSync.Client.UI
{
    internal static class GameInputBlocker
    {
        ...
        public static void Block(Action<string> log)
        {
            ...
            if (type == null)
            {
                log("[SptModSync] Could not find Unity's EventSystem - the sync window will not block " +
            ...
            try
            {
                ...
                log($"[SptModSync] Game menu input suspended while the sync window is open ({found.Length} event system(s)).");
            }
            catch (Exception ex)
            {
                log($"[SptModSync] Could not suspend game menu input: {ex.Message}");
            }
        }
        ...
        public static void Unblock(Action<string> log)
        {
            ...
            _disabled = null;
            log("[SptModSync] Game menu input restored.");
        }
