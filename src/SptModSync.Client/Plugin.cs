...
using SptModSync.Client.Sync;
using SptModSync.Client.UI;
using SptModSync.Shared.Logging;
using SptModSync.Shared.Paths;
using UnityEngine;

namespace SptModSync.Client
{
    [BepInPlugin(PluginGuid, PluginName, PluginVersion)]
    public sealed class Plugin : BaseUnityPlugin
    {
        public const string PluginGuid = "com.thecrimsonfuckr.sptmodsync.client";
        public const string PluginName = "SPT Mod Sync";
        ...
        /// SptModSync.Client.log, so a sync issue can be debugged from one file without wading
        ...
        private void Awake()
        {
            ...
            _fileLog = new FileLog(Path.Combine(pluginDir, "SptModSync.Client.log"));
            ...
            if (_resetExclusions.Value)
            {
                ...
                LogInfo($"[SptModSync] ResetExclusionsOnNextLaunch was set - cleared {cleared} exclusion(s).");
            }
            ...
            if (_isHeadless)
            {
                ...
                LogInfo($"[SptModSync] Headless client detected ({headlessReason}) - syncing " +
            ...
            {
                LogInfo($"[SptModSync] Found {_previouslyStaged.Operations.Count} file(s) staged in a " +
        ...
        private async Task RunStartupCheckAsync()
        {
            try
            {
                LogInfo("[SptModSync] Checking server for updates...");
                var (manifest, config, diff) = await _orchestrator.CheckAsync();

                if (diff.Count == 0)
                {
                    LogInfo("[SptModSync] Nothing to sync.");
                    return;
                }

                LogInfo($"[SptModSync] {diff.Count} item(s) proposed.");
                ...
                _window.OnClose = () => LogInfo("[SptModSync] Sync skipped for this session.");
                _window.ExcludedCount = _orchestrator.CountExclusions();
                _window.ServerSptVersion = manifest.SptVersion;
                _window.OnResetExclusions = () =>
                {
                    var cleared = _orchestrator.ResetExclusions();
                    LogInfo($"[SptModSync] Cleared {cleared} exclusion(s); re-checking.");
                    _window.Close();
                    _ = RunStartupCheckAsync();
                };
            ...
            {
                LogWarning($"[SptModSync] Sync check failed - {SyncOrchestrator.Describe(ex)}");
                LogWarning("[SptModSync] Continuing without syncing.");
            }
        }
            ...
            System.Collections.Generic.HashSet<string> acceptedPaths)
        {
            ...
            if (pending == null)
            {
                LogInfo("[SptModSync] Headless: nothing needed downloading.");
                return;
            }

            LogInfo($"[SptModSync] Headless: applying {pending.Operations.Count} operation(s) and restarting.");
            ...
            System.Collections.Generic.HashSet<string> acceptedPaths)
        {
            ...
            {
                LogError($"[SptModSync] Download failed: {ex}");
            ...
            if (pending == null)
            {
                LogInfo("[SptModSync] Nothing needed downloading; config updated.");
            ...
            LogInfo($"[SptModSync] Download complete. {pending.Operations.Count} operation(s) staged, " +
                    "awaiting confirmation to apply.");

            _window.OnConfirmRestart = () =>
            {
                LogInfo("[SptModSync] Restart confirmed. Launching updater and closing game.");
                _orchestrator.CommitAndLaunchUpdater(pending);
                _stagedStore.Discard();
            ...
            {
                LogInfo("[SptModSync] Restart declined; keeping downloads for next launch.");
