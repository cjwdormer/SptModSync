...
public class GlobMatcherTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "TCFModSyncGlobTest_" + Guid.NewGuid().ToString("N"));
        ...
        CreateFile("TCFModSync.Updater.exe");
    ...
    public void DefaultPatterns_FindModsInPluginsFolder()
    {
        ...
        Assert.Contains("TCFModSync.Updater.exe", files);
    }
